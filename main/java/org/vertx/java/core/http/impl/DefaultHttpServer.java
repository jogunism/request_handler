...
public class DefaultHttpServer implements HttpServer {
	...
	public HttpServer listen(int port, String host) {
		...
		synchronized (vertx.sharedHttpServers()) {
			...
			if (shared == null) {
				...
				ChannelFactory factory = new NioServerSocketChannelFactory(vertx.getAcceptorPool(), availableWorkers);
				...
				bootstrap.setPipelineFactory(new ChannelPipelineFactory() {
					...
					public ChannelPipeline getPipeline() {
						...
						// ide state handler added to close idle channel connections.
						pipeline.addLast("idleStateHandler", new IdleStateHandler(timer, 30, 30, 0));
						pipeline.addLast("closeChannelHandler", new CloseChannelHandler());
			...
			if (requestHandler != null) {
				// Share the event loop thread to also serve the HttpServer's
				// network traffic.
	...
	public class CloseChannelHandler extends IdleStateAwareChannelHandler {
		...
		public void channelIdle(ChannelHandlerContext ctx, IdleStateEvent e) {
			if (e.getState() == IdleState.READER_IDLE) {
				...
				//log.info("Writer idle, closing channel");
				e.getChannel().close();
			}
		}
	}
	...
	public void close(final Handler<Void> done) {
		...
		synchronized (vertx.sharedHttpServers()) {
			...
			if (actualServer != null) {
				if (requestHandler != null) {
					actualServer.reqHandlerManager.removeHandler(
							requestHandler, ctx);
				}
				if (wsHandler != null) {
				...
				if (actualServer.reqHandlerManager.hasHandlers()
						|| actualServer.wsHandlerManager.hasHandlers()) {
					// The actual server still has handlers so we don't actually
					// close it
					if (done != null) {
						executeCloseDone(ctx, done);
					}
					...
					// The done handler needs to be executed on the context that
					// calls close, NOT the context
	...
	private void executeCloseDone(final Context closeContext,
	...
	public class ServerHandler extends SimpleChannelUpstreamHandler {
		...
		public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
			...
			if (msg instanceof HttpRequest) {
				...
				if (WEBSOCKET.equalsIgnoreCase(request
						.getHeader(HttpHeaders.Names.UPGRADE))) {
					// As a fun part, Firefox 6.0.2 supports Websockets protocol
					// '7'. But,
					// it doesn't send a normal 'Connection: Upgrade' header.
					// Instead it
		...
		public void exceptionCaught(ChannelHandlerContext ctx, ExceptionEvent e) throws Exception {
			...
			if (conn != null && t instanceof Exception) {
				...
				// Ignore - any exceptions not associated with any sock (e.g.
				// failure in ssl handshake) will
